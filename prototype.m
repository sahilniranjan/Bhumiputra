% Project Title: BHUMIPUTRA

clc
close all 
clear var

[filename, pathname] = uigetfile({'*.*';'*.jpg'}, 'Pick a Leaf Image');
...
% Otsu Segmentation feature(binarization is used as a pre-processor before OCR)
I_Otsu = imbinarize(I,graythresh(I));
% Conversion to HIS feature(Converts an RGB image to HSI)
I_HSI = rgb2hsi(I);
%% Extract Features
...
cp = classperf(groups);
fitcsvm = fitcsvm(data(train,:),groups(train),'showplot',false,'kernel_function','linear');
classes = ClassificationSVM(fitcsvm,data(test,:),'showplot',false);
